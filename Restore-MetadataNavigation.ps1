...
Function Recreate-MetadataNavigation(){
    $session = Get-SPTaxonomySession -Site $site
    $termStore = $session.TermStores[0]
    $group = $termStore.GetSiteCollectionGroup($site.Url)
...
Function Assign-MetadataNavigation(){
    ...
    Catch{
        Write-Error "Something was wrong assingning metadata navigation"
    }
    Finally{
...
Recreate-MetadataNavigation
Assign-MetadataNavigation
